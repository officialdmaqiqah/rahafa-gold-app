...
interface LivePriceBoardProps {
  ...
  trendPoints?: { date: string; day: string; price: number }[];
...
export function LivePriceBoard({
  ...
  // Real dynamic trend calculation from Supabase
  const activeTrend = trendPoints && trendPoints.length >= 2 
    ? trendPoints 
    : [
        { date: "", day: "7 Hari Lalu", price: Math.round(baseBuyPerGram * 0.99) },
        { date: "", day: "Hari ini", price: baseBuyPerGram }
      ];

  const trendPrices = activeTrend.map((p) => p.price);
  const minPrice = Math.min(...trendPrices);
  const maxPrice = Math.max(...trendPrices);
  const priceRange = maxPrice - minPrice || 1;

  // ViewBox: 500 wide, 120 tall. Margins: X: 24 to 476 (range 452). Y: 18 to 98 (range 80)
  const svgCoords = activeTrend.map((p, idx) => {
    const x = activeTrend.length === 1 
      ? 250 
      : Math.round(24 + (idx / (activeTrend.length - 1)) * 452);
    const normalized = priceRange === 0 ? 0.5 : (p.price - minPrice) / priceRange;
    const y = Math.round(98 - normalized * 80);
    return { x, y, ...p };
  });

  const polylinePoints = svgCoords.map((pt) => `${pt.x},${pt.y}`).join(" ");
  const polygonPoints = svgCoords.length > 0 
    ? `${svgCoords[0].x},120 ${polylinePoints} ${svgCoords[svgCoords.length - 1].x},120` 
    : "";
    ...
    <section id="harga-tren" className="py-16 bg-slate-50 border-t border-slate-200/60">
      <div className="max-w-[1360px] mx-auto px-4 sm:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500 bg-white px-4 py-2.5 rounded-xl border border-slate-200 shadow-xs">
            <Clock className="w-4 h-4 text-amber-500 flex-shrink-0" />
            <span>Sesi Terkini: <strong className="text-slate-800">{lastUpdatedText}</strong></span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
          ...
          <div className="lg:col-span-5 bg-gradient-to-br from-[#1b355a] to-[#122540] rounded-2xl p-6 text-white shadow-lg border border-[#2c4b78] flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`text-xs font-semibold flex items-center gap-1 ${trendPercentage >= 0 ? "text-emerald-300" : "text-rose-300"}`}>
                  {trendPercentage >= 0 ? (
                    <ArrowUpRight className="w-3.5 h-3.5" />
                  ) : (
                    <ArrowDownRight className="w-3.5 h-3.5" />
                  )}
                  {trendPercentage >= 0 ? `+${trendPercentage}%` : `${trendPercentage}%`} ({activeTrend.length} Sesi Terakhir)
                </span>
              </div>
          ...
          <div className="lg:col-span-7 bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-slate-900 text-base">Grafik Tren Riwayat Harga ({activeTrend.length} Sesi Terakhir)</h3>
                </div>
              ...
              <div className="h-44 w-full relative pt-4">
                <svg className="w-full h-full overflow-visible" viewBox="0 0 500 120" preserveAspectRatio="none">
                  ...
                  {svgCoords.map((pt, idx) => {
                    const isLast = idx === svgCoords.length - 1;
                    return (
                      <circle 
                        key={idx} 
                        cx={pt.x} 
                        cy={pt.y} 
                        r={isLast ? 5.5 : 4} 
                        fill={isLast ? "#f59e0b" : "#1b355a"} 
                        stroke={isLast ? "#ffffff" : "#d99b00"} 
                        strokeWidth="2" 
                      />
                    );
                  })}
                ...
                <div className="flex justify-between items-center text-[11px] font-semibold text-slate-400 mt-3 pt-2 border-t border-slate-100">
                  {activeTrend.map((p, idx) => (
                    <div key={idx} className="text-center">
                      <span className={idx === activeTrend.length - 1 ? "text-[#1b355a] font-bold" : ""}>
                        {p.day}
                      </span>
                    </div>
                  ))}
            ...
            <div className="pt-4 mt-2 flex items-center justify-between text-xs text-slate-600 bg-slate-50 p-3 rounded-xl">
              <div>
                Terendah: <strong className="text-slate-800">{formatRupiah(minPrice)}</strong>
              </div>
              <div>
                Tertinggi: <strong className="text-[#1b355a]">{formatRupiah(maxPrice)}</strong>
              </div>
              <div className={`font-bold flex items-center gap-1 ${trendPercentage >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
                {trendPercentage >= 0 ? (
                  <>
                    <TrendingUp className="w-3.5 h-3.5" />
                    <span>Tren Positif (+{trendPercentage}%)</span>
                  </>
                ) : (
                  <>
                    <TrendingDown className="w-3.5 h-3.5" />
                    <span>Tren Koreksi ({trendPercentage}%)</span>
                  </>
                )}
              </div>
            </div>
